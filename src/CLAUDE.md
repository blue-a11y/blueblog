...
- **所有数据请求统一使用 ahooks 的 `useRequest`**，禁止自行封装 useState + useEffect 请求模式：
  ...
  useEffect(() => { fetchBlogList().then(setData) }, [])
  ```
...
- 相关类型与导出它的组件放在一起
- **JSX 内容较多时，拆分为 render 变量或子渲染函数来组合**，保持主 return 简洁：
  ```tsx
  const header = (
    <header className="...">
      <Title />
  ...
  const renderList = () => items.map(item => <Item key={item.id} {...item} />)
...
- **条件渲染统一风格**：
  ...
  - 组件内有多个守卫条件时用 early return，减少嵌套：
    ...
    return (
      <div>
        {!user ? <Empty /> : loading ? <Spinner /> : <Content user={user} />}
      </div>
    )
    ```
